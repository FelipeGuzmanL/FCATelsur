...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form action="{{route('cables.update_cable', $detalles)}}" method="post" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group">
                                        ...
                                        @foreach ( $gravedad as $grave )
                                                <option value="{{ $grave->id }}" {{$grave->id == $detalles->alerta->id_gravedad ? 'selected' : ''}}>{{ $grave->gravedad }}</option>
                                        @endforeach
                                        </select>
                                      </div>
                                </div>
